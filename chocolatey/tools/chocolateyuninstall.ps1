...
$key = Get-UninstallRegistryKey -SoftwareName 'AppGeek*'
...
elseif ($key.Count -eq 0) {
  Write-Warning 'AppGeek is not installed, or was installed outside of Chocolatey. Nothing to do.'
}
else {
  Write-Warning "$($key.Count) matches found for AppGeek. Remove it manually rather than guessing:"
